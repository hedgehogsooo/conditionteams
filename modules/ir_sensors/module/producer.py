...
def generate_data_IR_sensors():
    """Имитирует поведение ИК-датчиков, установленных на автомобиле."""

    ir_data = [
        "Данные с ик-датчиков"
    ]

    while True:
        if not get_emulation_status():
            # print("[IR_SENSORS_INFO] Emulation status OFF — skipping data sending")
            sleep(random.randint(5, 10))
            continue

        ir_datas = {
            "ir_sensors_view" : (ir_data)
        }

        print("[IR_SENSOR_DEBUG] Data:", ir_datas)

        proceed_to_deliver(uuid4().__str__(), {
            "deliver_to": "data_processing",
            "operation": "data_ir_sensors",
            "ir_datas": ir_datas
        })
...
def producer_job(_, config, requests_queue: multiprocessing.Queue):
    ...
    threading.Thread(target=generate_data_IR_sensors).start()

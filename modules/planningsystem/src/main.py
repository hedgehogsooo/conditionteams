...
CONNECTION_URL = "http://connection:6005/receive_route_sheet"
TSODD_URL = "http://tsodd:8003"
...
class PlanningSystem:
    ...
    def receive_route_sheet(self, route_sheet):
        ...
        truck_id = route_sheet.get('truck_id', 'truck_123')
    ...
    def send_approved_route_to_connection(self):
        if not self.route_sheet:
            return False
        try:
            response = requests.post(CONNECTION_URL, json=self.route_sheet, timeout=5, headers={"Content-Type": "application/json"})
            if response.ok:

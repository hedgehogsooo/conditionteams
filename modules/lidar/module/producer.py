...
def generate_data_lidar():

    """Имитирует поведение лидара, установленного на автомобиле."""
    ...
    while True:
        ...
        data_lidar = {
            "lidar_view": (lidar_data)
        }

        print("[LIDAR_DEBUG] Data:", data_lidar)

        proceed_to_deliver(uuid4().__str__(), {
            "deliver_to": "data_processing",
            "operation": "data_lidars",
...
def producer_job(_, config, requests_queue: multiprocessing.Queue):
    ...
    threading.Thread(target=generate_data_lidar).start()
